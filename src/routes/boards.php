...
# projects
$app->get("/boards", $authorize($app), function () use ($app) {

    $configs = $app->container->get('configs');
    $securityContext = $_SESSION['securityContext'];
    $db = $app->container->get('db');
    $boardService = new Boards($db, $configs, $securityContext);

    $boards = $boardService->getBoards();

    foreach ($boards as $i=>$board) {
        $boards[$i] = $boardService->hydrateProject($board);
    }
    ...
    $templateVars = array(
        ...
        "section" => "boards.index",
        "boards" => $boards
    );

    $app->render(
        'pages/boards.html.twig',
        $templateVars,
        200
    );
});
